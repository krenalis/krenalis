...
// TestUpgradeLegacyPipelineEventMetadata verifies migration from the former
// 25-character event metadata columns and constraints.
func TestUpgradeLegacyPipelineEventMetadata(t *testing.T) {
	...
	_, err := database.Exec(t.Context(), `
		...
		FROM organizations LIMIT 1;
		INSERT INTO connections (id, workspace, connector, role, kms_encrypted_settings_key)
		VALUES ('333333333333', '222222222222', 'dummy', 'Destination', '\x');
		INSERT INTO pipelines (id, connection, target, event_type, ordering_group, delivery_endpoint,
			transformation_language, matching_in, matching_out, table_key, update_on_duplicates)
		VALUES ('444444444444', '333333333333', 'Event', 'contactCreated', 'contacts', 'contacts',
			'JavaScript', '', '', '', false)`)
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, column := range []struct {
		...
		err = database.QueryRow(t.Context(), `
			...
			WHERE table_schema = current_schema() AND table_name = 'pipelines'
	...
	for _, constraint := range []string{
		"pipelines_event_type_check", "pipelines_ordering_group_check", "pipelines_delivery_endpoint_check",
	} {
		assertConstraintDoesNotExist(t, database, "pipelines", constraint)
	}

	var eventType, orderingGroup, deliveryEndpoint string
	err = database.QueryRow(t.Context(), `
		SELECT event_type, ordering_group, delivery_endpoint FROM pipelines
		WHERE id = '444444444444'`).Scan(&eventType, &orderingGroup, &deliveryEndpoint)
	if err != nil {
		t.Fatal(err)
	}
	if eventType != "contactCreated" || orderingGroup != "contacts" || deliveryEndpoint != "contacts" {
		t.Fatalf("expected preserved event metadata, got %q, %q, %q", eventType, orderingGroup, deliveryEndpoint)
	}

	_, err = database.Exec(t.Context(), `UPDATE pipelines SET event_type = repeat('界', 30) WHERE id = '444444444444'`)
	if err != nil {
		t.Fatalf("expected a 30-rune event type to be accepted, got %v", err)
	}
