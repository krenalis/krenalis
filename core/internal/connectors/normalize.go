		...
		if valid {
			...
			if values := typ.Values(); values != nil {
				if !slices.Contains(values, v) {
					return nil, inputValidationErrorf(name, "contains an unsupported value %q", util.Abbreviate(v, 20))
				}
				...
				if l, ok := typ.ByteLen(); ok && len(v) > l {
					return nil, inputValidationErrorf(name, "has value %q that is longer than %d bytes", util.Abbreviate(v, 20), l)
				}
				if l, ok := typ.CharLen(); ok && utf8.RuneCountInString(v) > l {
					return nil, inputValidationErrorf(name, "has value %q that is longer than %d characters", util.Abbreviate(v, 20), l)
				}
			}
			value = v
		}
