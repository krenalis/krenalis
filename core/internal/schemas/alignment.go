...
func CheckAlignment(t1, t2 types.Type, exportMode *state.ExportMode) error {
	if t1.Kind() == types.InvalidKind {
		...
		for path, p := range t2.Properties().WalkAll() {
			if *exportMode != state.UpdateOnly && p.CreateRequired {
				return &Error{Msg: fmt.Sprintf("«%s» property is required for creation", path)}
			}
			if *exportMode != state.CreateOnly && p.UpdateRequired {
				return &Error{Msg: fmt.Sprintf("«%s» property is required for update", path)}
			}
		}
		return nil
	}
	if t2.Kind() == types.InvalidKind {
		for _, p := range t1.Properties().All() {
			return &Error{Msg: fmt.Sprintf("«%s» property no longer exists", p.Name)}
		}
	}
...
func checkTypeAlignment(name string, t1, t2 types.Type, exportMode *state.ExportMode) error {
	...
	if k1 == types.ObjectKind {
		if k2 != types.ObjectKind {
			return &Error{Msg: fmt.Sprintf("«%s» property's type has changed from «object» to «%s»", name, t2)}
		}
		...
		for _, p1 := range properties1.All() {
			...
			if !ok {
				return &Error{Msg: fmt.Sprintf("«%s» property no longer exists", path)}
			}
			if exportMode != nil {
				if p1.CreateRequired != p2.CreateRequired && *exportMode != state.UpdateOnly {
					if p1.CreateRequired {
						return &Error{Msg: fmt.Sprintf("«%s» property was previously required for creation but is no longer", path)}
					}
					return &Error{Msg: fmt.Sprintf("«%s» property was not previously required for creation but it is now required", path)}
				}
				if p1.UpdateRequired != p2.UpdateRequired && *exportMode != state.CreateOnly {
					if p1.UpdateRequired {
						return &Error{Msg: fmt.Sprintf("«%s» property was previously required for the update but is no longer", path)}
					}
					return &Error{Msg: fmt.Sprintf("«%s» property was not previously required for the update but it is now required", path)}
				}
			}
			if exportMode == nil && p1.ReadOptional != p2.ReadOptional {
				if p1.ReadOptional {
					return &Error{Msg: fmt.Sprintf("«%s» property was previously optional but it is now non-optional", path)}
				}
				return &Error{Msg: fmt.Sprintf("«%s» property was previously non-optional but it is now optional", path)}
			}
			if p1.Nullable != p2.Nullable {
				if p1.Nullable {
					return &Error{Msg: fmt.Sprintf("«%s» property was previously nullable but it is no longer nullable", path)}
				}
				return &Error{Msg: fmt.Sprintf("«%s» property was previously non-nullable but is now nullable", path)}
			}
	...
	case types.StringKind:
		...
		if c1 != c2 || ok1 != ok2 {
			...
			return &Error{Msg: fmt.Sprintf("character length of the «%s» property's type has changed from %s to %s", name, v1, v2)}
		}
		b1, ok1 := t1.MaxBytes()
		b2, ok2 := t2.MaxBytes()
		if b1 != b2 || ok1 != ok2 {
			...
			return &Error{Msg: fmt.Sprintf("byte length of the «%s» property's type has changed from %s to %s", name, v1, v2)}
		}
		vs1 := t1.Values()
		vs2 := t2.Values()
		if vs1 != nil && vs2 == nil {
			return &Error{Msg: fmt.Sprintf("«%s» property was previously limited to specific values but is now unrestricted", name)}
		}
		if vs1 == nil && vs2 != nil {
			return &Error{Msg: fmt.Sprintf("«%s» property was previously unrestricted but is now limited to specific values", name)}
		}
		for _, v1 := range vs1 {
	...
	case types.IntKind:
		if t1.IsUnsigned() != t2.IsUnsigned() {
			if t1.IsUnsigned() {
				return &Error{Msg: fmt.Sprintf("«%s» property's type has changed from «unsigned %s» to «%s»", name, t1, t2)}
			}
			return &Error{Msg: fmt.Sprintf("«%s» property's type has changed from «%s» to «unsigned %s»", name, t1, t2)}
		}
		...
		if t1.IsUnsigned() {
			...
			return &Error{Msg: fmt.Sprintf("range of the «%s» property's type has changed from [%d,%d] to [%d,%d]", name, min1, max1, min2, max2)}
		}
	...
	case types.FloatKind:
		...
		if t1.IsReal() && !t2.IsReal() {
			return &Error{Msg: fmt.Sprintf("«%s» property's type has changed from real to non-real", name)}
		}
		if !t1.IsReal() && t2.IsReal() {
			return &Error{Msg: fmt.Sprintf("«%s» property's type has changed from non-real to real", name)}
		}
		min1, max1 := t1.FloatRange()
		min2, max2 := t2.FloatRange()
		return &Error{Msg: fmt.Sprintf("range of the «%s» property's type has changed from [%g,%g] to [%g,%g]", name, min1, max1, min2, max2)}
	case types.DecimalKind:
		p1 := t1.Precision()
		p2 := t2.Precision()
		if p1 != p2 {
			return &Error{Msg: fmt.Sprintf("precision of the «%s» property's type has changed from %d to %d", name, p1, p2)}
		}
		s1 := t1.Scale()
		s2 := t2.Scale()
		if s1 != s2 {
			return &Error{Msg: fmt.Sprintf("scale of the «%s» property's type has changed from %d to %d", name, s1, s2)}
		}
		min1, max1 := t1.DecimalRange()
		min2, max2 := t2.DecimalRange()
		return &Error{Msg: fmt.Sprintf("range of «%s» property's type has changed from [%s,%s] to [%s,%s]", name, min1, max1, min2, max2)}
	case types.ArrayKind:
		...
		return &Error{Msg: fmt.Sprintf("«%s» property elements were not required to be unique, but now it is required", name)}
	case types.MapKind:
