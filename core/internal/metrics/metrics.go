...
func (c *Collector) ExportProfileConsentFailed(pipeline string, count int) {
	c.Failed(ExportProfileConsentStep, pipeline, count, "")
}

// ExportProfileConsentPassed increases the passed count for the
// ExportProfileConsent step and pipeline by the given count. It is safe to call
// concurrently from multiple goroutines.
func (c *Collector) ExportProfileConsentPassed(pipeline string, count int) {
	c.Passed(ExportProfileConsentStep, pipeline, count)
}
...
// concurrently from multiple goroutines.
func (c *Collector) ImportProfileConsentFailed(pipeline string, count int) {
	c.Failed(ImportProfileConsentStep, pipeline, count, "")
}

// ImportProfileConsentPassed increases the passed count for the
// ImportProfileConsent step and pipeline by the given count. It is safe to call
// concurrently from multiple goroutines.
func (c *Collector) ImportProfileConsentPassed(pipeline string, count int) {
	c.Passed(ImportProfileConsentStep, pipeline, count)
}
...
func (c *Collector) store(timeslot int32, metrics map[string]*metrics) {
	...
	c.buf.Reset()
	c.buf.WriteString("WITH t(organization, workspace, connection, pipeline, target, timeslot, passed_0, passed_1, passed_2, passed_3, passed_4, passed_5, passed_6, passed_7, passed_8, failed_0, failed_1, failed_2, failed_3, failed_4, failed_5, failed_6, failed_7, failed_8) AS (\n\tVALUES ")
	i := 0
