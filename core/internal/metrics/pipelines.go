...
func (c *Collector) queryMetrics(ctx context.Context, start, end time.Time, resolution time.Duration, selection Selection) (Metrics, error) {
	...
	query.WriteString("timeslot/$1 AS slot, SUM(passed_0), SUM(passed_1), SUM(passed_2), SUM(passed_3), SUM(passed_4), SUM(passed_5), SUM(passed_6), SUM(passed_7), SUM(passed_8)," +
		" SUM(failed_0), SUM(failed_1), SUM(failed_2), SUM(failed_3), SUM(failed_4), SUM(failed_5), SUM(failed_6), SUM(failed_7), SUM(failed_8)\n" +
	...
	for rows.Next() {
		...
		err = rows.Scan(&id, &slot,
			&passed[0], &passed[1], &passed[2], &passed[3], &passed[4], &passed[5], &passed[6], &passed[7], &passed[8],
			&failed[0], &failed[1], &failed[2], &failed[3], &failed[4], &failed[5], &failed[6], &failed[7], &failed[8])
		if err != nil {
			return Metrics{}, err
		}
