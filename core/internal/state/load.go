...
func (state *State) load(ctx context.Context, oauthCredentials map[string]*OAuthCredentials) error {
	...
	err = tx.QueryScan(ctx, "SELECT id, name, enabled, members_limit, access_keys_limit, workspaces_limit,"+
		" connectors_limit, connections_limit, pipelines_limit, workspace_requests_rate_per_minute, workspace_requests_burst_capacity,"+
		" workspace_events_rate_per_minute, workspace_events_burst_capacity, organization_requests_rate_per_minute, organization_requests_burst_capacity FROM organizations", func(rows *db.Rows) error {
		for rows.Next() {
			...
			if err := rows.Scan(&org.ID, &org.Name, &org.Enabled, &limits.Members, &limits.AccessKeys,
				...
				&limits.Rates.Events.RatePerMinute, &limits.Rates.Events.BurstCapacity,
				&limits.Rates.Organization.RatePerMinute, &limits.Rates.Organization.BurstCapacity); err != nil {
