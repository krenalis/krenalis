...
func (state *State) CookieKeys(ctx context.Context) ([]byte, []byte, error) {
	...
	if len(dataKey) != 64 {
		clear(dataKey)
		return nil, nil, errors.New("cookie key is not 64 bytes long")
	}
	return dataKey[:32], dataKey[32:], nil
}
...
func (state *State) RequestIDKey(ctx context.Context) ([]byte, error) {
	dataKey, err := state.cipher.KMS().DecryptDataKey(ctx, state.metadata.kmsEncryptedRequestIDKey)
	if err != nil {
		return nil, err
	}
	if len(dataKey) != 32 {
		clear(dataKey)
		return nil, errors.New("request ID key is not 32 bytes long")
	}
	return dataKey, nil
}
