...
func TestValuerStringerConsistency(t *testing.T) {
	tests := []struct {
		...
		{"SimulatedAccountPreparing", SimulatedAccountPreparing, "Preparing"},
		{"SimulatedAccountReady", SimulatedAccountReady, "Ready"},
		{"SimulatedAccountFailed", SimulatedAccountFailed, "Failed"},
...
func TestValuerStringerInvalidValues(t *testing.T) {
	tests := []struct {
		...
		{"SimulatedAccountStatus", SimulatedAccountStatus(-1)},
		{"Health", Health(-1)},
		{"Role", Role(-1)},
		{"Target", Target(-1)},
		{"Language", Language(-1)},
