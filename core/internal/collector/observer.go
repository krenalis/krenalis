...
type listener struct {
	...
	requiredConsentsLogical state.RequiredConsentsLogical
...
// requiredConsents is non-empty, only events that have the required consents
// will be observed, according to requiredConsentsLogical.
//
...
// listeners.
func (observer *Observer) CreateListener(connections []string, size int, filter *state.Where, requiredConsents []string, requiredConsentsLogical state.RequiredConsentsLogical) (string, error) {
	id := uuid.New().String()
	listener := listener{
		id:                      id,
...
func (observer *Observer) addEvent(event events.Event) {
	...
	for _, listener := range observer.listeners {
		...
		if !consents.Satisfies(listener.requiredConsents, listener.requiredConsentsLogical != state.ConsentsOr, event) {
			continue
		}
		listener.Lock()
		var p int
