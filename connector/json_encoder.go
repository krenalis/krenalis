...
// MarshalJSON returns the JSON encoding of the value v of type t.
func MarshalJSON(v any, t types.Type) []byte {
	enc := newJSONEncoder(false, false, false)
	return enc.Append(nil, t, v)
}

// An jsonEncoder writes JSON values to a slice of bytes.
...
func newJSONEncoder(indent, generateASCII, allowSpecialFloats bool) *jsonEncoder {
	return &jsonEncoder{
		indent:             indent,
...
// extended buffer.
func (enc *jsonEncoder) Append(b []byte, t types.Type, v any) []byte {
	if v == nil {
	...
	case types.ObjectKind:
		...
		case map[string]any:
			for i, p := range t.Properties() {
				...
				b = enc.Append(b, p.Type, v[p.Name])
			}
		}
		enc.Depth--
		if enc.indent {
...
func (enc *jsonEncoder) sortKeys(v map[string]any) []string {
...
func (enc *jsonEncoder) appendJSONValue(b []byte, v any) []byte {
	if v == nil {
		return append(b, "null"...)
	}
	switch v := v.(type) {
...
// enc.Depth.
func (enc *jsonEncoder) AppendIndentation(b []byte) []byte {
	b = append(b, '\n')
...
func (enc *jsonEncoder) appendFloat(b []byte, f float64, bits int) []byte {
...
func (enc *jsonEncoder) appendString(b []byte, s string) []byte {
	b = append(b, '"')
	last := 0
	for i, r := range s {
