...
5. In the **Path** field, enter the path of the Parquet file, relative to the storage root path. The absolute path will be displayed so you can verify that the path you have entered is correct.
6. (Optional) Proceed with the other fields:
    * **Compression**: Compression format. Select a format if you want the Parquet file to be further compressed.
