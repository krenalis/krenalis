...
If such property / column does not exist in the schema of the imported user data, or if the type is not supported, the Business ID is not imported.
...
Until that moment, the identities will continue to show the Business ID value present at the time when the import of such identities occurred.