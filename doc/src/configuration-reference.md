...
| `MEERGO_HTTP_EXTERNAL_EVENT_URL`  | `/api/v1/events` (same server) | URL of the endpoint receiving events. If not set, assumed to be `/api/v1/events` on the same server. Example: `https://meergo.example.com:8080/api/v1/events`.                                                     |
