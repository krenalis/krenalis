...
func TestActionsCreation(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			err: `unexpected HTTP status code 400: {"error":{"code":"BadRequest","message":"column name for the unique ID is mandatory"}}`,
		},
		{
			conn: srcFsID,
			action: chichitester.ActionToSet{
		...
		},
		{
			...
			action: chichitester.ActionToSet{
				...
				UniqueIDColumn:  "my_id_column",
				TimestampColumn: "timestamp",
			},
			err: `unexpected HTTP status code 400: {"error":{"code":"BadRequest","message":"unique ID column \"my_id_column\" not found within input schema"}}`,
		},
		{
			conn: postgreSQLConnection,
			action: chichitester.ActionToSet{
