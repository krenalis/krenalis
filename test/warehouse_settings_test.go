...
func TestWarehouseSettings(t *testing.T) {
	...
	const expectedErr = `unexpected HTTP status code 422: {"error":{"code":"WarehouseNonInitializable","message":"data warehouse is not initializable: expected an empty database, got: table '_destinations_users', index '_destinations_users_pkey', table '_operations', sequence '_operations_id_seq', index '_operations_pkey', table '_user_identities', sequence '_user_identities___cluster___seq', sequence '_user_identities___pk___seq', index '_user_identities_pkey', table '_users_0', table 'events', index 'events_pkey', view 'users'"}}`
