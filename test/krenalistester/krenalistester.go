...
// [PopulateProfileSchema], [SetFileSystemRoot], [SetSyntheticPhotosDir], and
// [SetNATSAckWait]) and
...
func NewKrenalisInstance(t *testing.T) *Krenalis {
	...
	var err error
	k.repo, err = filepath.Abs("../")
	if err != nil {
		t.Fatal(err)
	}
	_, err = os.Stat(filepath.Join(k.repo, "go.mod"))
	if err != nil {
	...
	if needToGenerateAssets {
		k.assetsGenerated = make(chan struct{}, 1)
		go func() {
	...
	k.transformationsTempDir = transformationsTempDir
...
// is considered true.
func (k *Krenalis) PopulateProfileSchema(populate bool) {
	k.populateProfileSchema = populate
}
...
// SetSyntheticPhotosDir sets the verified photo catalog directory before Start.
func (k *Krenalis) SetSyntheticPhotosDir(directory string) {
...
func (k *Krenalis) Start() {
	...
	go func() {
		...
		k.stopNATSContainer = func() error {
	...
	k.t.Log("waiting for the containers to be started")
	containersStarted.Wait()
	k.t.Log("containers started")

	// Create the HTTP client.
	jar, err := cookiejar.New(nil)
	if err != nil {
		k.t.Fatalf("cannot create a cookie jar: %s", err)
	}
	...
	if launchKrenalisExternally {
		...
		krenalisDir := filepath.Join(k.repo, "test", "krenalis-executable-for-tests")
		...
		go func() {
			...
			assets := os.DirFS(filepath.Join(k.repo, "admin", "assets"))
			{
	...
	for {
		...
		case <-k.krenalisRunning:
			k.t.Fatalf("Krenalis has exited")
		default:
		}
		...
		if err != nil {
			attempts++
			if attempts >= 30 {
				k.t.Fatalf("cannot connect to Krenalis on %q. No response after %d connections attempts, aborting test", addr, attempts)
			}
	...
	err = k.login()
	if err != nil {
		k.t.Fatalf("cannot log in into the API: %s", err)
	}
...
func (k *Krenalis) CountEventsInWarehouse(ctx context.Context) int {
	pool, err := ConnectionPool(ctx, testsSettings.Warehouse)
	if err != nil {
		k.t.Fatalf("cannot open warehouse for executing query tests: %s", err)
	}
	defer pool.Close()
	var count int
	err = pool.QueryRow(ctx, `SELECT COUNT(*) AS "count" FROM "events"`).Scan(&count)
	if err != nil {
		k.t.Fatalf("cannot execute count query: %s", err)
	}
	return count
}

// Stop stops the execution of Krenalis.
func (k *Krenalis) Stop() {
	if k.cancel != nil {
		k.cancel()
	}
	if k.krenalisRunning != nil {
		<-k.krenalisRunning
	}
	if k.transformationsTempDir == "" {
		panic("BUG: transformationsTempDir not set")
	}
	err := os.RemoveAll(k.transformationsTempDir)
	if err != nil {
		log.Printf("cannot remove transformations temporary directory: %s", err)
		return
	}
	if k.stopNATSContainer != nil {
		err := k.stopNATSContainer()
		if err != nil {
			log.Printf("cannot stop NATS container: %s", err)
		}
	}
	if k.stopPostgresContainer != nil {
		err := k.stopPostgresContainer()
		if err != nil {
			log.Printf("cannot stop PostgreSQL container: %s", err)
		}
	}
	if k.stopWarehouseContainer != nil {
		err := k.stopWarehouseContainer()
		if err != nil {
...
func (k *Krenalis) ExecQueryTestDatabase(ctx context.Context, query string, args ...any) {
	pool, err := ConnectionPool(ctx, testsSettings.Database)
	if err != nil {
		k.t.Fatalf("cannot establish connection to database: %s", err)
	}
	defer pool.Close()
	_, err = pool.Exec(ctx, query, args...)
	if err != nil {
		k.t.Fatalf("query %q failed: %s", query, err)
	}
}

// QueryRowTestDatabase queries a row on the test database, scanning it into
// dest, which must be a pointer.
func (k *Krenalis) QueryRowTestDatabase(ctx context.Context, dest any, query string, args ...any) {
	...
	if err != nil {
		k.t.Fatalf("cannot scan result of QueryRow: %s", err)
	}
}

// WorkspaceID returns the ID of the workspace on which the instance of Krenalis
// operates.
func (k *Krenalis) WorkspaceID() string {
	return k.ws
}

// SetWorkspaceID selects the workspace used by subsequent harness API calls.
func (k *Krenalis) SetWorkspaceID(id string) {
	if id == "" {
