...
func init() {
	...
	queryParameter := types.Property{
		...
		CreateRequired: true,
		Prefilled:      `"SELECT name, email, updated_at FROM customers WHERE ${last_change_time}"`,
				...
				Response: &Response{
					Parameters: []types.Property{
						...
						},
						{
							...
							Prefilled:   `"User"`,
							Description: "The entity on which the action operates. It is always `\"User\"` when the action imports users from a database.",
						},
						{
							Name:        "enabled",
							Type:        types.Boolean(),
