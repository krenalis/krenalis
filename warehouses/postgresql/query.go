...
func (warehouse *PostgreSQL) Count(ctx context.Context, table string, joins []warehouses.Join, where warehouses.Expr) (int, error) {
	...
	var total int
	err = pool.QueryRow(ctx, statement).Scan(&total)
	if err != nil {
		return 0, err
	}
