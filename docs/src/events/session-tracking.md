...
The session management functions depend on the specific SDK. The example below uses the [JavaScript SDK](/sources/javascript-sdk), but consult the SDK documentation for comprehensive details.

The Meergo [JavaScript SDK](/sources/javascript-sdk) automatically manages sessions unless specified otherwise. When a user visits your website for the first time, a session starts with a default timeout of 30 minutes.
...
This behavior is the default setting. To disable it, set the `autoTrack` option to `false` when loading the [JavaScript SDK](/sources/javascript-sdk):
...
The default session timeout is 30 minutes. If you wish to set a different timeout, specify the `timeout` option with a value in milliseconds when loading the [JavaScript SDK](/sources/javascript-sdk):
