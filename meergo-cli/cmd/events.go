...
func init() {
	...
	eventsCmd = &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			defer cancel()
			listener := meergoapis.AddEventListener(ws, size, source, server, stream)
			defer meergoapis.RemoveEventListener(ws, listener)
			...
			for {
				...
				events, discarded := meergoapis.Events(ws, listener)
