...
func (visualization *Visualization) smartEventToSQL(which any) (string, bool, error) {
	...
	if len(smartEvent.Pages) > 0 {
		...
		for i, page := range smartEvent.Pages {
			...
			if page.Domain != "" {
				where.WriteString(" AND `domain` = " + postgres.QuoteValue(page.Domain))
			}
			where.WriteString(")")
		}
		where.WriteString(")")
	...
	if len(smartEvent.Buttons) > 0 {
		...
		for i, button := range smartEvent.Buttons {
			...
			where.WriteString("(" + cond)
			if button.Domain != "" {
				where.WriteString(" AND `domain` = " + postgres.QuoteValue(button.Domain))
			}
			where.WriteString(")")
		}
		where.WriteString(")")
...
func conditionToSQL(condition Condition) (string, error) {
	...
	where += " " + postgres.QuoteValue(condition.Value)
	if condition.Domain != "" {
		where += fmt.Sprintf(" AND `domain` = %s", postgres.QuoteValue(condition.Domain))
	}
	return where, nil
}
