...
func Test_normalize(t *testing.T) {
	...
	tests := []struct {
		...
		{types.Int(16), stdjson.Number("-6"), -6, nil},
		{types.Int(16), stdjson.Number("-6.0"), -6, nil},
		// Int(32).
		{types.Int(32), -9261, -9261, nil},
		{types.Int(32), -9261.0, -9261, nil},
		{types.Int(32), stdjson.Number("-9261"), -9261, nil},
		{types.Int(32), stdjson.Number("-9261.0"), -9261, nil},
		// Uint(8).
		{types.Uint(8), uint(3), uint(3), nil},
		{types.Uint(8), 3.0, uint(3), nil},
		{types.Uint(8), stdjson.Number("3"), uint(3), nil},
		{types.Uint(8), stdjson.Number("3.0"), uint(3), nil},
		// Uint(32).
		{types.Uint(32), uint(47303), uint(47303), nil},
		{types.Uint(32), 47303.0, uint(47303), nil},
		{types.Uint(32), stdjson.Number("47303"), uint(47303), nil},
		{types.Uint(32), stdjson.Number("47303.0"), uint(47303), nil},
		// Float(32).
		{types.Float(32), float64(float32(12.79)), float64(float32(12.79)), nil},
		{types.Float(32), stdjson.Number("12"), float64(float32(12.0)), nil},
		{types.Float(32), stdjson.Number("12.79"), float64(float32(12.79)), nil},
		{types.Float(32), math.NaN(), math.NaN(), nil},
		// Float(64).
		{types.Float(64), 12.7902743017496882, 12.7902743017496882, nil},
		{types.Float(64), stdjson.Number("12"), 12.0, nil},
		{types.Float(64), stdjson.Number("12.79027430174968829"), 12.79027430174968829, nil},
		...
		{types.Decimal(10, 3), 6.639, decimal.NewFromFloat(6.639), nil},
		{types.Decimal(10, 3), stdjson.Number("6.639"), decimal.NewFromFloat(6.639), nil},
		// DateTime.
		...
		{types.DateTime(), stdjson.Number(strconv.FormatInt(aDateTime.Unix(), 10)), time.Date(2023, 5, 3, 15, 47, 22, 0, time.UTC), &state.TimeLayouts{DateTime: "unix"}},
		// Date.
		...
		{types.Year(), stdjson.Number("2023"), 2023, nil},
		// UUID.
		{types.UUID(), "123e4567-e89b-12d3-a456-426614174000", "123e4567-e89b-12d3-a456-426614174000", nil},
		// JSON.
		{types.JSON(), stdjson.RawMessage(`{"a": 5}`), json.Value(`{"a": 5}`), nil},
		{types.JSON(), stdjson.RawMessage(` { "a" : 5 } `), json.Value(` { "a" : 5 } `), nil},
		{types.JSON(), stdjson.RawMessage(" \t503\n"), json.Value(" \t503\n"), nil},
		{types.JSON(), stdjson.Number("302"), json.Value("302"), nil},
		{types.JSON(), []byte(`{"a": 5}`), json.Value(`{"a": 5}`), nil},
		{types.JSON(), `{"a": 5}`, json.Value(`"{\"a\": 5}"`), nil},
		{types.JSON(), map[string]any{"a": 5.0}, json.Value(`{"a":5}`), nil},
		// Inet.
		...
		{types.Array(types.Int(32)), []any{stdjson.Number("1.0"), stdjson.Number("2.0")}, []any{1, 2}, nil},
...
func Test_validJSON(t *testing.T) {
	...
	tests := []struct {
		...
		{23.985, true},
		{[]any{1.0, "a", true}, true},
		{[]any{1.0, "a", stdjson.RawMessage(`{"foo":[1,2,3]}`)}, false},
		{map[string]any{"": "", "a": 5.4}, true},
		{map[string]any{"\xff": "foo"}, false},
		{map[string]any{"foo": "\xff"}, false},
		{math.NaN(), false},
		{math.Inf(-1), false},
		{stdjson.Number(""), false},
		{stdjson.Number("515"), true},
		{stdjson.Number(" 515"), false},
		{stdjson.Number("-34.96"), true},
		{stdjson.Number("+34.96"), false},
		{stdjson.Number("true"), false},
	}
