...
func (state *State) Load() error {
	...
	err := state.db.Transaction(ctx, func(tx *postgres.Tx) error {
		...
		err = state.db.QueryScan(ctx, "SELECT id, connection, target, event_type, name, enabled, schedule_start,\n"+
			"schedule_period, in_schema, out_schema, filter, mapping, transformation_source, transformation_version,\n"+
			"identifiers, query, path, table_name, sheet, (user_cursor).id, (user_cursor).timestamp,\n"+
			"(user_cursor).next, health, export_mode, matching_properties_internal, matching_properties_external\n"+
			"FROM actions",
			func(rows *postgres.Rows) error {
				for rows.Next() {
					var connectionID int
					...
					err := rows.Scan(&action.ID, &connectionID, &action.Target, &eventType, &action.Name,
						...
						&filter, &mapping, &transformation.Source, &transformation.Version, &action.Identifiers,
