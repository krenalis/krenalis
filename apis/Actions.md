...
| Source **Database** on **Users / Groups**                  | —                                           | Generated by executing the query                     | `users` schema (with no meta properties)  | Query, identity column and (optionally) a timestamp column                                                           |
| Source **Storage** on **Users / Groups**                   | —                                           | Read from file columns / headers                     | `users` schema (with no meta properties)  | File path and sheet, connector, connector settings, compression, identity column and (optionally) a timestamp column |
...
| Destination **Storage** on **Users / Groups**              | Filters the users to write to the file      | —                                                    | —                                         | File path and sheet                                                                                                  |
...
graph LR
	...
	source --> source_storage(["Storage"]) --> source_storage_users(["Users 🟢"])
	...
	dest --> dest_storage(["Storage"]) --> dest_storage_users(["Users 🔴"])
