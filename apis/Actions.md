...
| Source **Database** on **Users / Groups**                  | —                                           | Generated by executing the query                     | `users` schema (with no meta properties)  | Query, unique ID column and (optionally) a timestamp column                                                           |
| Source **FileStorage** on **Users / Groups**               | —                                           | Read from file columns / headers                     | `users` schema (with no meta properties)  | File path and sheet, connector, connector settings, compression, unique ID column and (optionally) a timestamp column |
