...
| **App** (destination) on Events                          | Filters the events to map and then send     | Events schema without GID (hard-coded into Chichi) | From the Event Type (may be invalid to indicate there is no schema)       | -                                                                                                     |
...
| **Mobile / Server / Website** (source) on Users / Groups | -                                           | Events schema without GID (hard-coded into Chichi) | `users_identities` (with no meta-properties)                              | Transformation function is not allowed, only mappings are                                             |
