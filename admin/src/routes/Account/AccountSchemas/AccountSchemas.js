...
export default class AccountSchemas extends React.Component {
	...
	handleSchemaSaving = async () => {
		let [, err] = await call('/admin/schemas/update', {SchemaName: this.state.schemaName, Schema: this.state.newSchema});
		if (err !== null) {
			this.setState({status: {variant:'danger', icon:'exclamation-octagon', text:err}});
			this.toast.current.toast();
			return;
		...
		return (
			<div className='AccountSchemas'>
				...
				<div className='content'>
					...
					<div className='bar'>
						<SlSelect name='schemaSelector' value={this.state.schemaName} onSlChange={(e) => {this.handleSchemaNameUpdate(e)}}>
