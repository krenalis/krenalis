...
export default class Dashboard extends React.Component {
	...
	render() {
		let suggestions = [];
		for (let s of suggestionsData)
			suggestions.push(
				<QuerySuggestion description={s.description} query={s.jsonQuery} onClick={this.applySuggestion} />
			);
		let columnsCells = [];
		...
		for (let type of chartTypes) {
			chartOptions.push(
				<option value={type} selected={type === this.state.chartType ? true : false}>
					{type}
				</option>
			);
		}
			...
			<div className='Dashboard'>
				<div className='content'>
					...
					{this.state.statusMessage && (
						<StatusMessage onClose={this.closeStatusMessage} message={this.state.statusMessage} />
					)}
					...
					<div className='editor'>
						<div className='textarea'>
							...
							<div
								...
								style={{ gridTemplateColumns: `repeat(${this.state.columns.length}, 1fr)` }}
							>
								{columnsCells}
							</div>
						</div>
					...
					<SlideOver
						onClose={this.toggleSlideOver}
						title={'Note aggiuntive'}
						isOpen={this.state.isSlideOverOpen ? true : false}
					>
						{additionalNotesData}
					</SlideOver>
				</div>
