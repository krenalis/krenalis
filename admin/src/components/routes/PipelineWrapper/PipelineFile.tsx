...
const PipelineFile = () => {
	...
	useEffect(() => {
		const fetchFields = async () => {
			...
			if (isEditing && !isFormatChanged) {
				try {
					ui = await api.workspaces.connections.pipelineUiEvent(pipeline.id, 'load', null);
				} catch (err) {
					setTimeout(() => setIsFormatLoading(false), 300);
	...
	return (
		...
		>
			<SlSelect
				label='Format'
				className='pipeline__file-format'
				value={String(pipeline.format)}
				onSlChange={onFormatChange}
			>
				{pipeline.format !== '' && (
					<div className='pipeline__file-format-logo' slot='prefix'>
			...
			{isFormatLoading ? (
				...
				pipeline.format !== '' && (
					<div className='pipeline__file-settings'>
						<FileSettings
							hasSheets={hasSheets}
...
const FileSettings = ({ hasSheets, fileExtension, fileFields, pathInputRef }: FileSettingsProps) => {
	...
	const orderByError = useMemo<string>(() => {
		...
		const property = pipeline.orderBy;
	...
	useEffect(() => {
		pathRef.current = {
			...pathRef.current,
			lastConfirmation: pipeline.path,
			lastUpdate: pipeline.path,
		};
		sheetRef.current = {
			lastConfirmation: pipeline.sheet,
			lastUpdate: pipeline.sheet,
		};
		compressionRef.current = {
			lastConfirmation: pipeline.compression,
			lastUpdate: pipeline.compression,
		};
		settingsRef.current = {
	...
	useEffect(() => {
		if (isImport || isEditing || pipelineType.target !== 'User') {
			return;
		}
		const hasEmailColumn = pipelineType.inputSchema.properties.find((p) => p.name === 'email');
		if (hasEmailColumn) {
			const p = { ...pipeline };
			p.orderBy = 'email';
			setPipeline(p);
	...
	useEffect(() => {
		if (isEditing) {
			if (pipeline.path === '') {
				...
				// pipeline, so the pipeline path is now empty.
				return;
			}
			fetchPath(pipeline.path);
		}
	}, []);
	...
	const loadSheets = async () => {
		...
		p.sheet = '';
		setPipeline(p);
		sheetsSelectRef.current.classList.add('pipeline__file-sheets--hide-list-box'); // prevent the listbox from flashing.
		setSheets({});
		...
		try {
			...
			setTimeout(() => {
				...
				sheetsSelectRef.current.classList.remove('pipeline__file-sheets--hide-list-box');
				setHasSheetsError(true);
	...
	const onConfirmFile = async () => {
		...
		if (schema == null) {
			...
			setTimeout(() => {
				setIssues(res.issues);
				const pipelineTyp = { ...pipelineType };
				pipelineTyp.inputSchema = schema;
				setPipelineType(pipelineTyp);
	...
	const records = async (limit: number, isConfirmation?: boolean) => {
		...
		try {
			res = await api.workspaces.connections.records(
				...
				pipeline.path,
				pipeline.format,
				pipeline.sheet === undefined ? null : pipeline.sheet,
				pipeline.compression,
				settings,
				limit,
			);
			...
			handleError(err);
			const isAlreadyConfirmed = pipelineType.inputSchema != null;
		...
		if (isConfirmation) {
			pathRef.current.lastConfirmation = pipeline.path;
			compressionRef.current.lastConfirmation = pipeline.compression;
			settingsRef.current.lastConfirmation = { ...settings };
			if (pipeline.sheet != null) {
				sheetRef.current.lastConfirmation = pipeline.sheet;
			}
	...
	return (
		<div>
			<div className='pipeline__file-path-wrapper'>
				<SlInput
					className='pipeline__file-path'
					name='path'
					value={pipeline.path!}
					type='text'
					...
					ref={pathInputRef}
				>
					<div className='pipeline__file-path-label' slot='label'>
						<div className='pipeline__file-path-text'>File path</div>
						<div className='pipeline__file-path-description'>
				...
				<div
					className={`pipeline__file-complete-path-error${absolutePathError !== '' ? ' pipeline__file-complete-path-error--visible' : ''}`}
				>
					{absolutePathError}
				</div>
				<div
					className={`pipeline__file-complete-path${absolutePath !== '' ? ' pipeline__file-complete-path--visible' : ''}`}
				>
			...
			{hasSheets &&
				(connection.role === 'Source' ? (
					<div className='pipeline__file-sheets-wrapper'>
						<SlSelect
							onSlFocus={onSheetsLoad}
							className='pipeline__file-sheets'
							ref={sheetsSelectRef}
							name='sheet'
							value={
								pipeline.sheet == null
									? undefined
									: Object.keys(sheets).find((k) => sheets[k] === pipeline.sheet)
							}
							label='Sheet'
					...
					<SlInput
						className='pipeline__file-sheets-input'
						name='input'
			...
			<SlSelect
				className='pipeline__file-compression'
				name='compression'
				value={pipeline.compression}
				label='Compression'
			...
			{pipelineType.fields.includes('OrderBy') && (
				<div className='pipeline__file-ordering'>
					<Combobox
						value={pipeline.orderBy}
						onInput={onOrderByChange}
						onSelect={onOrderBySelect}
						items={getOrderingPropertyPathComboboxItems(pipelineType.inputSchema)}
			...
			{isImport && (
				<div className='pipeline__file-buttons'>
					<SlButton
					...
					<FeedbackButton
						...
						className='pipeline__file-confirm'
						variant='success'
						size='small'
			...
			>
				<div className='pipeline__file-preview-drawer-label' slot='label'>
