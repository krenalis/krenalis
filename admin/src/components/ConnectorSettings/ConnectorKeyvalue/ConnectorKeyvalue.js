					...
					{this.state.rows.map((row) => {
						return (
							<div className="row" data-id={row.id} key={row.id}>
								<div className="key cell">{renderConnectorComponent(this.props.keyComponent, this.onKeyChange, row.key)}</div>
								<div className="value cell">{renderConnectorComponent(this.props.valueComponent, this.onValueChange, row.value)}</div>
								{row.id !== 1 && <SlIcon className="removeRow" name='trash3' onClick={() => this.onRemoveRowClick(row.id)} />}
							</div>
						)
					})}
				</div>
