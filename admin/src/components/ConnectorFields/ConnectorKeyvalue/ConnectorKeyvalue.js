...
const ConnectorKeyValue = ({
	...
	let [rows, setRows] = useState(initRows(val));
	...
	const formatRows = (rws) => {
		let formatted = {};
		for (let r of rws) formatted[r.key] = r.value;
		return formatted;
	};

	const onAddRowClick = () => {
		let rws = [...rows, { id: rows[rows.length - 1].id + 1, key: '', value: '' }];
		setRows(rws);
	};

	const onRemoveRowClick = (id) => {
		let rws = [...rows];
		let filtered = rws.filter((r) => r.id !== id);
		setRows(filtered);
